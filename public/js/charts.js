...
class ChartManager {
    ...
    applyDarkTheme(config) {
        ...
        if (!darkConfig.options.plugins.legend) darkConfig.options.plugins.legend = {};
        if (!darkConfig.options.plugins.tooltip) darkConfig.options.plugins.tooltip = {};
    ...
    createPerformanceChart(canvasId, data) {
        const config = {
            type: 'bar',
            data: {
                labels: data.models || ['GPT-4o', 'Gemini-Pro', 'Claude-3.5', 'GPT-4-Turbo', 'Deepseek-R1'],
                datasets: [{
                    label: 'Vitórias',
    ...
    createWinRateChart(canvasId, data) {
        const config = {
            type: 'doughnut',
            data: {
                labels: data.labels || ['Vitórias Brancas', 'Vitórias Pretas', 'Empates'],
                datasets: [{
    ...
    createOpeningChart(canvasId, data) {
        const config = {
            type: 'bar',
            data: {
                labels: data.openings || ['1.e4', '1.d4', '1.Nf3', '1.c4', 'Sicilian', 'French'],
                datasets: [{
                    label: 'Win Rate %',
    ...
    createAccuracyChart(canvasId, data) {
        const config = {
            type: 'radar',
            data: {
                labels: data.categories || ['Abertura', 'Meio-jogo', 'Final', 'Tática', 'Estratégia'],
    ...
    createColorPerformanceChart(canvasId, data) {
        const config = {
            type: 'bar',
            data: {
                labels: data.models || ['GPT-4o', 'Gemini-Pro', 'Claude-3.5', 'GPT-4-Turbo'],
                datasets: [{
                    label: 'Brancas %',
...
window.createChart = (canvasId, config) => chartManager.createChart(canvasId, config);
window.updateChart = (canvasId, newData) => chartManager.updateChart(canvasId, newData);
