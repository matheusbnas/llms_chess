...
class PGNViewer {
  ...
  async loadAvailableGames() {
    try {
      const response = await fetch("/api/arena/matchups");
  ...
  async loadGamesForMatchup() {
    ...
    try {
      ...
      const response = await fetch(`/api/arena/matchups/${matchup}/games`);
    ...
    if (!matchup || !gameFile) {
      if (window.arena) {
        window.arena.showToast(
          "Selecione um confronto e uma partida",
          "warning"
        );
      }
      return;
  ...
  updateGameInfo() {
    ...
    document.getElementById("game-duration").textContent =
      this.formatDuration(headers.TimeControl) || "-";
    document.getElementById("game-date").textContent =
      this.formatDate(headers.Date) || "-";
    document.getElementById("game-event").textContent =
      headers.Event || "LLM Chess Arena";
  }
  ...
  updateMoveHistory() {
      ...
      html += `
                <div style="padding: 4px 8px; border-bottom: 1px solid var(--border-color); display: flex; align-items: center;">
                    ...
                    <span class="${isWhiteActive ? "active-move" : ""}" 
                          style="margin-right: 12px; flex: 1; cursor: pointer; padding: 2px 4px; border-radius: 3px; ${
                            isWhiteActive
                              ? "background: var(--lichess-blue); color: white;"
                              : ""
                          }" 
                          onclick="pgnViewer.goToMove(${
                            i + 1
                          })">${whiteMove}</span>
                    <span class="${isBlackActive ? "active-move" : ""}" 
                          style="flex: 1; cursor: pointer; padding: 2px 4px; border-radius: 3px; ${
                            isBlackActive
                              ? "background: var(--lichess-blue); color: white;"
                              : ""
                          }" 
    ...
    document.getElementById(
      "move-count"
    ).textContent = `${this.moves.length} lances`;
  }
  ...
  updateNavigationControls() {
    ...
    ).textContent = `Lance ${this.currentMove} de ${this.maxMoves}`;
    document.getElementById(
      "max-moves-display"
    ).textContent = `Lance ${this.maxMoves}`;
  ...
  updateCurrentMoveInfo() {
    if (this.currentMove === 0) {
      document.getElementById("current-move-san").textContent =
        "Posição inicial";
    ...
    if (move) {
      ...
      ).textContent = `${moveNumber}. ${color} - ${move.from || ""} → ${
        move.to || ""
      }`;
