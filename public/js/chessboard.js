...
class Chessboard {
    ...
    possibleMoves.forEach((moveSquareId) => {
      ...
      if (square) {
        ...
        square.classList.add(
          hasTargetPiece ? "possible-capture" : "possible-move"
        );
      }
    });
  }
  ...
  isValidPawnMove(fromSquare, toSquare, piece) {
    ...
    if (fileDiff === 0) {
      ...
      if (rankDiff === 2 * direction && fromRank === startRank) {
        return (
          !this.getPiece(toSquare) &&
          !this.getPiece(fromSquare.charAt(0) + (fromRank + direction))
        );
      }
    }
  ...
  // Adicionar método à classe Chessboard
